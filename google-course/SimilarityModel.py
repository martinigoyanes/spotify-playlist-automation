...
choc_data = pd.read_csv(
    "https://download.mlcc.google.com/mledu-datasets/flavors_of_cacao.csv", sep=",", encoding='latin-1')

# We can rename the columns.
choc_data.columns = ['maker', 'specific_origin', 'reference_number', 'review_date',
...
choc_data['specific_origin'] = choc_data['specific_origin'].str.replace(
...
choc_data['broad_origin'] = choc_data['broad_origin'].str.replace(
    '.', '').apply(cleanup_spelling_abbrev)
...
choc_data['maker'] = choc_data['maker'].str.replace(
    u'Na\xef\xbf\xbdve', 'Naive')
    ...
    def __init__(self,
        ...
        def create_tower(features, columns):
            ...
            for layer_idx, layer_output_dim in enumerate(hidden_dims):
                ...
                if inspect:
                    self._vars_to_inspect['hidden_layer_%d' %
                                          layer_idx] = hidden_layer
            ...
            for ofn in output_feature_names:
                ...
                output_layer[ofn] = {
                    ...
                    'logits':
                        # w / 10.), bias)
                        tf.add(tf.matmul(hidden_layer, w), bias)
                }
                if inspect:
                ...
                if (i % 10 == 0) or i == num_iterations:
                    print('\riteration%6d,   ' % i + ',   '.join(
                        ['%s %s %s: %7.3f' % (k0, k1, k2, v2)
                         ...
                         for k1, v1 in six.iteritems(v0)
                         for k2, v2 in six.iteritems(v1)]), end=" "
                    )
                    if plot_results:
                        iterations.append(i)
                        for k0, v0 in six.iteritems(results):
