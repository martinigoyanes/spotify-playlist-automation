...
def get_user_tokens(uid):
    user_tokens = User.objects.filter(uid=uid)
...
def is_spotify_authenticated(uid):
    tokens = get_user_tokens(uid)
    if tokens:
        expiry = tokens.expires_in
        if expiry <= timezone.now():
            refresh_spotify_token(uid)
...
def refresh_spotify_token(uid):
    refresh_token = get_user_tokens(uid).refresh_token
